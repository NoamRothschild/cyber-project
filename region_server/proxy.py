...
async def create_proxy(src_node_pos: Tuple[int, int], dst_node_pos: Tuple[int, int], event: region_net.ProxyEvent) -> None:
...
async def remove_proxy(src_node_pos: Tuple[int, int], dst_node_pos: Tuple[int, int], sender_id: int, session_id: int) -> None:
    from nodes import nodes
    from region_node import RegionNode
    if dst_node := nodes.get(dst_node_pos):
